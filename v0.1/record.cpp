...
//	system("ps -ef|grep aaa.flv | grep -v grep | cut -c 9-15|xargs kill -9");
...
int main(int argc, char **argv)
{
	...
	string applicationpath = argv[3];
	string ffmpegpath = argv[4];
	string tempstr;
	if(argc>5){
		tempstr = argv[5];
		getvidoeinfo(tempstr);
	}
	if(argc>6){
		tempstr = argv[6];
		getvidoeinfo(tempstr);
	...
	//cout << "bitrate=" << bitrate << ", imageresolution=" << imageresolution << ", framerate=" << framerate << endl;


//    system("ps -ef | grep cc | grep -v grep | cut -c 9-15|xargs kill -9");
    string basepath = applicationpath + "/streamfile";
//    delete_file(basepath.c_str());

    string Year, Month, Day, Hour, Minute, Second;
//    setOperationTime(Year, Month, Day, Hour, Minute, Second);
